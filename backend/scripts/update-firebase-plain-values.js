...
async function updateFirebaseConfigurationPlain() {
  try {
    console.log('🔥 Updating Firebase Configuration with Plain Values...\n');

    // Initialize DynamicConfigService
    const dynamicConfig = new DynamicConfigService(pool);
    await dynamicConfig.initialize();

    // Firebase configurations to update (as plain values, not JSON strings)
    const firebaseConfigs = {
      ...
      // Enable Firebase
      firebase_enabled: true, // Boolean value, not string
    };

    console.log('Updating Firebase configurations with plain values...\n');

    // Update each configuration using DynamicConfigService.set method
    let successCount = 0;
    let errorCount = 0;

    for (const [key, value] of Object.entries(firebaseConfigs)) {
      try {
        // Use DynamicConfigService.set which handles the proper formatting
        await dynamicConfig.set(key, value, 'system');
    ...
    // Clear cache and reload to test
    console.log('\n🔄 Reloading configuration cache...');
    await dynamicConfig.loadConfigurations();
    ...
    if (isClientValid && isAdminValid) {
      ...
      console.log('\n📝 What you can do now:');
      console.log('   1. Test Firebase authentication endpoints at /api/firebase-auth/*');
      ...
      console.log('\n🚀 Test endpoints:');
      console.log('   GET  /api/firebase-auth/status - Check Firebase service status');
      console.log('   POST /api/firebase-auth/verify-token - Verify Firebase ID tokens');
    } else {
      console.log('\n⚠️  Configuration updated but validation failed.');
      console.log(
        '   This might be normal if Firebase SDK initialization requires actual Firebase connection.'
      );
      console.log('   Try testing the endpoints to see if they work.');
    }
  } catch (error) {
...
updateFirebaseConfigurationPlain();
