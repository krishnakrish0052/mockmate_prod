    ...
    try {
      ...
      for (const userData of users) {
        ...
        `, [
          ...
          userData.total_sessions_completed, userData.total_spent_usd, userData.registration_source, 
          userData.country
        ]);
        ...
        if (userResult.rows.length > 0) {
          ...
          for (let i = 0; i < Math.min(userData.total_sessions_completed, 3); i++) {
            await db.query(`
      ...
      if (proUser.rows.length > 0) {
        try {
          ...
          console.log(`   ⚠️  Could not add payment: ${error.message}`);
        }
      }
      ...
      if (updateResult.rows.length > 0) {
        ...
        console.log('   ⚠️  No app versions updated (may not exist)');
      }
      ...
      if (checkCreditBalanceTable.rows[0].exists) {
        ...
        for (const user of users.rows) {
          try {
            ...
            console.log(`   ⚠️  Could not add credit balance for user: ${error.message}`);
            break;
          }
        }
