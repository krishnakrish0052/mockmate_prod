...
export const CliButton: React.FC<CliButtonProps> = ({
  ...
  const variantClasses = {
    ...
    danger:
      'bg-cli-black border-red-500 text-red-500 hover:bg-red-500 hover:text-cli-black hover:shadow-glow-danger',
    success:
      'bg-cli-black border-cli-green text-cli-green hover:bg-cli-green hover:text-cli-black hover:shadow-glow-success',
    warning:
      'bg-cli-black border-cli-amber text-cli-amber hover:bg-cli-amber hover:text-cli-black hover:shadow-glow-warning',
  };
...
export const CliBadge: React.FC<CliBadgeProps> = ({
  ...
  const variantClasses = {
    ...
    danger: 'bg-red-500 text-cli-white',
...
// CLI Select Component
export interface CliSelectProps extends React.SelectHTMLAttributes<HTMLSelectElement> {
  label?: string;
  error?: string;
  options: Array<{ value: string; label: string }>;
}

export const CliSelect: React.FC<CliSelectProps> = ({
  label,
  error,
  options,
  className = '',
    ...
    <div className='space-y-2'>
      ...
      <select
        className={`
          block w-full rounded-md border-2 border-cli-gray bg-cli-dark
          px-3 py-2
          font-mono text-cli-white transition-all duration-200
      ...
      >
        {options.map(option => (
          <option key={option.value} value={option.value} className='bg-cli-dark'>
            {option.label}
          </option>
        ))}
      </select>
...
// CLI Textarea Component
export interface CliTextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  label?: string;
  error?: string;
}

export const CliTextarea: React.FC<CliTextareaProps> = ({
  label,
  error,
  className = '',
  value,
    ...
    <div className='space-y-2'>
      ...
      <textarea
        className={`
          block w-full rounded-md border-2 border-cli-gray bg-cli-dark
          px-3 py-2 font-mono
          text-cli-white placeholder-cli-light-gray transition-all duration-200
          focus:border-primary-500 focus:outline-none
          focus:ring-2 focus:ring-primary-500
          focus:ring-offset-2 focus:ring-offset-cli-black
          ${error ? 'border-red-500' : ''}
